...
help:
	...
	@echo "  globe-drag-study  What a drag does near the limb"
...
# What a drag does near the limb (#330): whether the grabbed sky
# stays under the pointer, how fast the page moves per pixel of hand,
# what the solver says about its own footing, and whether a gesture
# that leaves the disc and returns brings the page back.
globe-drag-study: classes
	@echo "  globe drag"
	@mkdir -p docs/studies/globe-drag
	@$(JAVA) -Xmx1g -Djava.awt.headless=true \
		-cp "$(CLASSES_DIR):$(LIB_DIR)/*" juranometria.tool.globe.GlobeDragStudyMain \
		> docs/studies/globe-drag/measurements.md
	@echo "written to docs/studies/globe-drag/measurements.md"
