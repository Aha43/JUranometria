...
Neither can happen with the bundled pack, and the reason is structural rather than a survey of pages. At the widest field the atlas offers, the tallest page the assembler will build reaches **119.9°** from its centre - it letterboxes a taller window rather than promising sky the projection cannot draw honestly; the assembler queries that reach plus the pack's declared **5.39°** object margin; and nothing it returns extends more than that same **5.39°** from its own centre. So the furthest any boundary can lie from a page centre is 119.9 + 5.39 + 5.39 = **130.72°**, short of the 90° horizon. The largest object the pack actually records is 5.38°, and it holds 13,371 in all. Every number here is asked of production rather than restated: the margin from the pack's manifest, the reach from a page the assembler built.
