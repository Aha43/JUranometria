...
**49 files** touch process-wide state: 27 protected by the shared SwingSession guard, 21 restoring locally - the JUnit-fixture preference shape the consolidation (#224) deliberately kept - 0 unprotected, and the guard itself classified as what it is.
...
- files using doClick somewhere: 36
...
- reads of live chart state (currentScene, pageOffsetY, navigation.state): **386** across the suite
- explicit event-thread hand-offs (invokeAndWait): **664**
