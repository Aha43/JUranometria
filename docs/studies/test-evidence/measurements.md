...
| juranometria/app/ExportJourneyTest.java | preferences | protected-locally |
...
**44 files** touch process-wide state: 21 protected by the shared SwingSession guard, 22 restoring locally - the JUnit-fixture preference shape the consolidation (#224) deliberately kept - 0 unprotected, and the guard itself classified as what it is.
...
- explicit event-thread hand-offs (invokeAndWait): **586**

## Generated evidence, classified

| class | the contract | files |
|---|---|---|
| deterministic-report | regenerates byte-for-byte on the same tree | 17 |
