...
**46 files** touch process-wide state: 24 protected by the shared SwingSession guard, 21 restoring locally - the JUnit-fixture preference shape the consolidation (#224) deliberately kept - 0 unprotected, and the guard itself classified as what it is.
...
**9 evidence executables** touch process-wide state; 8 carry an unpaired touch.
...
**32 display-dependent files.** 17 state a focused-window or focus-owner premise, 24 state a reachability premise, and 19 use a back-door action (doClick or postActionEvent) somewhere - each either a menu convention or a debt the discipline issue #243 owns.
...
**6 files** name a look and feel, a shortcut mask or an operating system.
...
- explicit event-thread hand-offs (invokeAndWait): **627**

## Generated evidence, classified

| class | the contract | files |
|---|---|---|
| deterministic-report | regenerates byte-for-byte on the same tree | 18 |
