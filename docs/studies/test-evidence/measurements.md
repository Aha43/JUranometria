...
**48 files** touch process-wide state: 26 protected by the shared SwingSession guard, 21 restoring locally - the JUnit-fixture preference shape the consolidation (#224) deliberately kept - 0 unprotected, and the guard itself classified as what it is.
...
**34 display-dependent files.** 17 state a focused-window or focus-owner premise, 25 state a reachability premise, and 20 use a back-door action (doClick or postActionEvent) somewhere - each either a menu convention or a debt the discipline issue #243 owns.
...
**7 files** name a look and feel, a shortcut mask or an operating system.
...
- explicit event-thread hand-offs (invokeAndWait): **651**

## Generated evidence, classified

| class | the contract | files |
|---|---|---|
| deterministic-report | regenerates byte-for-byte on the same tree | 22 |
