...
**50 files** touch process-wide state: 27 protected by the shared SwingSession guard, 22 restoring locally - the JUnit-fixture preference shape the consolidation (#224) deliberately kept - 0 unprotected, and the guard itself classified as what it is.
...
**35 display-dependent files.** 18 state a focused-window or focus-owner premise, 26 state a reachability premise, and 21 use a back-door action (doClick or postActionEvent) somewhere - each either a menu convention or a debt the discipline issue #243 owns.
...
- reads of live chart state (currentScene, pageOffsetY, navigation.state): **390** across the suite
- explicit event-thread hand-offs (invokeAndWait): **666**

## Generated evidence, classified

| class | the contract | files |
|---|---|---|
| deterministic-report | regenerates byte-for-byte on the same tree | 49 |
