...
**45 files** touch process-wide state: 23 protected by the shared SwingSession guard, 21 restoring locally - the JUnit-fixture preference shape the consolidation (#224) deliberately kept - 0 unprotected, and the guard itself classified as what it is.
...
**31 display-dependent files.** 17 state a focused-window or focus-owner premise, 23 state a reachability premise, and 18 use a back-door action (doClick or postActionEvent) somewhere - each either a menu convention or a debt the discipline issue #243 owns.

## Input routes across the whole suite

- files dispatching real pointer events: 17
- files dispatching real keyboard events: 2
- files using doClick somewhere: 33
...
**5 files** name a look and feel, a shortcut mask or an operating system.
...
- explicit event-thread hand-offs (invokeAndWait): **605**
