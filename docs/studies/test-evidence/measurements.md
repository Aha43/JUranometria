...
**43 files** touch process-wide state: 21 protected by the shared SwingSession guard, 21 restoring locally - the JUnit-fixture preference shape the consolidation (#224) deliberately kept - 0 unprotected, and the guard itself classified as what it is.
...
**28 display-dependent files.** 16 state a focused-window or focus-owner premise, 20 state a reachability premise, and 17 use a back-door action (doClick or postActionEvent) somewhere - each either a menu convention or a debt the discipline issue #243 owns.

## Input routes across the whole suite

- files dispatching real pointer events: 17
- files dispatching real keyboard events: 2
- files using doClick somewhere: 31
...
- reads of live chart state (currentScene, pageOffsetY, navigation.state): **348** across the suite
- explicit event-thread hand-offs (invokeAndWait): **550**

## Generated evidence, classified

| class | the contract | files |
|---|---|---|
| deterministic-report | regenerates byte-for-byte on the same tree | 14 |
...
| renderer-drawn | byte-reproducible per machine; production ink, no widgets | 285 |
