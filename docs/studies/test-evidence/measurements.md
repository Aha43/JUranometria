...
**55 files** touch process-wide state: 28 protected by the shared SwingSession guard, 26 restoring locally - the JUnit-fixture preference shape the consolidation (#224) deliberately kept - 0 unprotected, and the guard itself classified as what it is.
...
- reads of live chart state (currentScene, pageOffsetY, navigation.state): **423** across the suite
- explicit event-thread hand-offs (invokeAndWait): **693**
