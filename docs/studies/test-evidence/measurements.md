...
**58 files** touch process-wide state: 30 protected by the shared SwingSession guard, 27 restoring locally - the JUnit-fixture preference shape the consolidation (#224) deliberately kept - 0 unprotected, and the guard itself classified as what it is.
...
**39 display-dependent files.** 19 state a focused-window or focus-owner premise, 27 state a reachability premise, and 21 use a back-door action (doClick or postActionEvent) somewhere - each either a menu convention or a debt the discipline issue #243 owns.
...
- explicit event-thread hand-offs (invokeAndWait): **714**
