...
**60 files** touch process-wide state: 31 protected by the shared SwingSession guard, 28 restoring locally - the JUnit-fixture preference shape the consolidation (#224) deliberately kept - 0 unprotected, and the guard itself classified as what it is.
...
**20 evidence executables** touch process-wide state; 19 carry an unpaired touch.
...
**43 display-dependent files.** 19 state a focused-window or focus-owner premise, 28 state a reachability premise, and 21 use a back-door action (doClick or postActionEvent) somewhere - each either a menu convention or a debt the discipline issue #243 owns.
...
- explicit event-thread hand-offs (invokeAndWait): **757**

## Generated evidence, classified

| class | the contract | files |
|---|---|---|
| deterministic-report | regenerates byte-for-byte on the same tree | 64 |
...
| renderer-drawn | byte-reproducible per machine; production ink, no widgets | 385 |
