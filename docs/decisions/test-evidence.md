...
this document to them: **66 files** touch process-wide state —
**33** use the shared guard, **32** restore locally,
...
button press; and #332 added the sprint's closing journey, which
walks a reader from Home up the field-width ladder to the globe and
back by pressing the toolbar's own controls, points at a star to
...
than passes where there is no display; and #348 added four to the
local-preference shape. The store's test, because the question it
...
had itself written to; the session's test, which proves the store is
read once at startup and never consulted again, and so must watch a
real store to count the reads; and the reader's own path, which
chooses a sky language, restarts, and exports — a restart being
precisely a second store over the same node, which a fixture cannot
be; and the Settings language controls, which assert that opening and
cancelling writes nothing — a claim only a real store can answer,
since a fixture that recorded nothing would satisfy it however the
dialog behaved. #349 then added the sky-language closing journey to
...
label's colour, which is what the first version of the test read. #350
also added the search language test: its result list is a popup window of
its own rather than a child of the field, so a walk through the field's
children finds the idle state and reports it as the whole surface. #350 added one more to the
...
reads of live chart state** against **804 explicit hand-offs**
...
each query to the field and reads the popup it opens on the event thread,
because a popup read off it is a popup read while it is still being
built; and for #350's toolbar language test, which
