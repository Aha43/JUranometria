...
- `docs/studies/wider-field/released-pages.txt` - every page the
  released atlas could draw, digested twice: the geometry it decided
  on, which is arithmetic and holds anywhere, and the rasterised
  page, which depends on the machine and is an oracle only on the one
  named in the file. The promise that the 1-36 degree atlas is
  untouched is therefore a standing contract rather than a claim.
  Regenerated by `make wider-field-study`.
...
- **Every page at 36 degrees and below**: eighty of them across ten
  ...
  what they draw and where, and identical to the pixel on the
  platform the rows were recorded on.
