...
- `docs/studies/wider-field/released-pages.txt` - every page the
  ...
  decided on, every vector operation it performed (the grid, the
  boundaries, the figures, the furniture, the ground and the
  labels), and the rasterised page. The first two are arithmetic and
  hold anywhere; the third depends on the font stack and the JDK's
  2D pipeline, and is an oracle only on the exact platform named in
  the file. The promise that the 1-36 degree atlas is untouched is
...
- **Every page at 36 degrees and below**: eighty of them across ten
  ...
  what they draw, where, and with what ink, and identical to the
  pixel on the platform the rows were recorded on.
