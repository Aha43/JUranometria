...
- **Reference circles are carried as curves** (issue #298). The atlas
  ...
  shape that is there, and draws it — a line, or an exact circular
  arc, in as many runs as the page cuts it into, and clipped to where
  there is sky rather than only to where there is paper. Still
  nothing a reader can see: at every field the atlas offers, the two
  answers differ by less than a tenth of a pixel.
...
- **File, Export Chart Sheet.** One item, one dialog, three formats:
  SVG to edit, PDF to print, PNG to send. The dialog asks only what
  the gate found worth asking - format, paper, resolution, and one
  switch for the working selection's transient marks - because the
  chart already decides everything else and a sheet is the chart.
  Defaults produce a useful observing-club sheet from a reader who
  knows nothing about pixels, dots per inch or projections.
- **PDF**, genuine vector at the paper's own size, with the labels as
  glyph outlines: the base-14 fonts every reader has cannot spell the
  chart's own notation, and embedding a font was out of scope for
  this sprint.
- **PNG**, the whole sheet at a stated resolution, with a `pHYs`
  chunk so a printer sizes it rather than fitting it to whatever page
  it has. It replays the sheet's own recorded ink rather than
  rendering the chart a second time, so a point on it is a point.
...
- **SVG, self-contained.** No stylesheet, script, font, image or network
  ...
  can be retyped in Inkscape; the sheet and the chart are named groups
  and each run of ink or labels carries its own class, so the two can
  be selected apart without any of it being reordered. The sheet states
...
- **A 42-degree field step**, the widest page the atlas offers. It
  ...
  from the printable-chart gate, which measured five candidate fields
  and chose on a stated budget: corner anisotropy no worse than 12%,
  half again the 8.1% the atlas already accepted at 36 degrees. That
  admits 40 and 42 and excludes 45 and 48. See
  [the printable-chart decision](docs/decisions/printable-chart.md).
- `docs/studies/wider-field/released-pages.txt` - every page the
  released atlas could draw, digested three ways: the marks it
  decided on, every shape it drew (the grid, the boundaries, the
  figures, the ground), and the rasterised page. The first two are
  arithmetic and hold anywhere; the third carries the labels too,
  which font metrics both place and select, so it depends on the
  machine and is an oracle only on the exact platform named in the
  file. The promise that the 1-36 degree atlas is untouched is
  therefore a standing contract rather than a claim. Regenerated by
  `make wider-field-study`.
...
- The recorder now carries a stroke whole - cap, join, miter and dash
  phase, not only width and dash - and every writer says as much of
  it as its format can. The SVG had been writing one blanket line cap
  for the whole sheet, which was not the cap half its ink was drawn
  with.
...
- **The projection.** The chart is gnomonic at 42 degrees exactly as
  it is at 36, because that is what lets the meridian, the horizon
  and the ecliptic be clipped to the paper analytically rather than
  sampled. The gate measured the stereographic alternative and
  rejected it.
- **Every page at 36 degrees and below**: eighty of them across ten
  field steps, four centres and both chart grounds - identical in
  what they draw, where, and in what ink, and identical to the pixel
  on the platform the rows were recorded on.
