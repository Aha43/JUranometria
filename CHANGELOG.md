...
- `docs/studies/wider-field/released-pages.txt` - every page the
  released atlas could draw, hashed from its own pixels, so the
  promise that the 1-36 degree atlas is untouched is a standing
  contract rather than a claim. Regenerated by `make
  wider-field-study` and checked by `make evidence-contracts`.
...
- **The projection.** The chart is gnomonic at 42 degrees exactly as
  ...
  rejected it.
- **Every page at 36 degrees and below**, to the pixel: eighty of
  them across ten field steps, four centres and both chart grounds.
